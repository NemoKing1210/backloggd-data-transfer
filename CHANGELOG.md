...
- Log import writes run in parallel (same concurrency setting as Read / Cleanup).
- Import progress shows active game chips and a parallel counter while writing.

## [0.6.9] - 2026-07-18

### Changed

- Cleanup multi-log scan uses the same parallel request pool as import (library pages + per-game log probes).

## [0.6.8] - 2026-07-18

### Changed

- Library indexing fetches different shelves and known pages in parallel (same concurrency setting as match lookups).

## [0.6.7] - 2026-07-18

### Fixed

- «Your log» no longer marks almost every game as existing: sidebar `[play_type]` buttons and empty log-editor chrome were false positives (e.g. Icarus).
